...
import {
  ...
  List, ListOrdered, Heading2, Heading3, Quote, Undo, Redo, Youtube as YTIcon
} from 'lucide-react'
  ...
  return (
    <button
      ...
      className={`p-2 rounded-lg text-sm transition-all disabled:opacity-30 ${
        active ? 'bg-purple-600 text-white shadow-lg' : 'hover:bg-white/10 text-white/60 hover:text-white'
      }`}
    >
      {children}
    </button>
...
export default function RichEditor({ content = '', onChange }) {
  ...
  const editor = useEditor({
    extensions: [
      StarterKit,
      Image.configure({ allowBase64: false, HTMLAttributes: { class: 'rounded-xl max-w-full' } }),
      Link.configure({ openOnClick: false, HTMLAttributes: { class: 'text-purple-400 underline' } }),
      Youtube.configure({ width: 640, height: 360, HTMLAttributes: { class: 'rounded-xl w-full aspect-video' } }),
    ],
    content,
  ...
  return (
    <div className="border border-white/10 rounded-2xl overflow-hidden focus-within:border-purple-500/50 transition-all bg-[#050505]">
      ...
      <div className="flex flex-wrap items-center gap-1 p-2 border-b border-white/5 bg-[#0d0d0d]">
        ...
        <ToolBtn onClick={() => editor.chain().focus().redo().run()} title="إعادة (Ctrl+Y)"
          disabled={!editor.can().redo()}>
          <Redo size={16} />
        </ToolBtn>
        {divider}
        <ToolBtn onClick={() => editor.chain().focus().toggleBold().run()}
          active={editor.isActive('bold')} title="غامق (Ctrl+B)">
          <Bold size={16} />
        </ToolBtn>
        <ToolBtn onClick={() => editor.chain().focus().toggleItalic().run()}
          active={editor.isActive('italic')} title="مائل (Ctrl+I)">
          <Italic size={16} />
        </ToolBtn>
        {divider}
        <ToolBtn onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()}
          active={editor.isActive('heading', { level: 2 })} title="عنوان H2">
          <Heading2 size={16} />
        </ToolBtn>
        <ToolBtn onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()}
          active={editor.isActive('heading', { level: 3 })} title="عنوان H3">
          <Heading3 size={16} />
        </ToolBtn>
        {divider}
        <ToolBtn onClick={() => editor.chain().focus().toggleBulletList().run()}
          active={editor.isActive('bulletList')} title="قائمة نقطية">
          <List size={16} />
        </ToolBtn>
        <ToolBtn onClick={() => editor.chain().focus().toggleOrderedList().run()}
          active={editor.isActive('orderedList')} title="قائمة مرقّمة">
          <ListOrdered size={16} />
        </ToolBtn>
        <ToolBtn onClick={() => editor.chain().focus().toggleBlockquote().run()}
          active={editor.isActive('blockquote')} title="اقتباس">
          <Quote size={16} />
        </ToolBtn>
        {divider}
        <ToolBtn onClick={setLink} active={editor.isActive('link')} title="رابط">
          <LinkIcon size={16} />
        </ToolBtn>
        <ToolBtn onClick={handleImageUpload} title="رفع صورة" disabled={uploading}>
          <ImageIcon size={16} />
        </ToolBtn>
        <ToolBtn onClick={addYoutube} title="فيديو يوتيوب">
          <span className="text-[10px] font-black">YT</span>
        </ToolBtn>
        {uploading && <span className="text-[10px] text-white/40 mr-2 font-black uppercase">جارٍ الرفع...</span>}
      </div>
