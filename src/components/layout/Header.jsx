...
import { 
  Search, Bell, Menu, X, LayoutDashboard, 
  User, LogOut, Moon, Sun, 
  PenSquare, Compass, BookOpen, Layers, Zap, Hash
} from 'lucide-react'
...
export default function Header() {
  ...
  const navLinks = [
    ...
    isAuthorOrAdmin && { to: '/studio/posts', label: 'مقالاتي', icon: BookOpen },
  ].filter(Boolean)
          ...
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="flex-shrink-0">
              <Link to="/" className="group flex items-center gap-3">
                 ...
                 <span className="text-2xl md:text-3xl font-black text-white tracking-tighter italic">مداد</span>
              </Link>
            </div>
            ...
            <div className="flex items-center gap-2 md:gap-3">
               ...
               <div className="flex items-center gap-1 md:gap-1.5 bg-white/5 p-1 rounded-2xl border border-white/5 shadow-xl">
                 <button onClick={() => setSearchOpen(true)} aria-label="البحث" className="p-2.5 text-white/40 hover:text-purple-400 hover:bg-purple-600/10 rounded-xl transition-all"><Search size={20} /></button>
                 {user && (<Suspense fallback={<div className="w-10 h-10" />}><NotificationCenter /></Suspense>)}
                 ...
                 {user && (
                    <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-purple-600/10 border border-purple-500/20 rounded-xl mx-2">
                      ...
                      <span className="text-[10px] font-black text-purple-400">{profile?.points || 0}</span>
                      <div className="w-px h-3 bg-purple-500/20 mx-1" />
                      <span className="text-[10px] font-black text-white/40">LVL {Math.floor((profile?.points || 0) / 100) + 1}</span>
                    </div>
                 )}
                 ...
                 {user ? (
                    ...
                    >
                      {profile?.avatar_url ? (
                        <OptimizedImage src={profile.avatar_url} alt={profile.full_name} width={40} height={40} />
                      ) : (
                        <div className="w-full h-full bg-purple-600 flex items-center justify-center text-white font-black">{profile?.full_name?.[0]}</div>
                      )}
                    </button>
                 ) : (
                   <Link to="/login" className="px-5 py-2 text-white/60 hover:text-white text-xs font-black transition-all">دخول</Link>
                 )}
               </div>

               <button onClick={() => setMobileMenuOpen(true)} aria-label="القائمة" className="lg:hidden p-2 text-white"><Menu size={28} /></button>
            </div>
          </div>
        </div>
      </header>
                ...
                <div className="w-10 h-10 rounded-full bg-purple-600/10 border border-purple-500/20 overflow-hidden flex items-center justify-center font-black text-purple-400 italic shrink-0 shadow-lg transition-all">
                 ...
                 <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                       ...
                       <p className="text-purple-400 text-[10px] font-black italic">مستوى {Math.floor((profile?.points || 0) / 100) + 1}</p>
                    </div>
                 </div>
              </div>
              <div className="space-y-1 p-1 text-right">
                <Link to="/studio" onClick={closeAllMenus} className="flex items-center gap-3 px-5 py-3.5 rounded-2xl text-xs font-black text-white/60 hover:text-white hover:bg-white/5 transition-all"><LayoutDashboard size={18} /> لوحة التحكم</Link>
                <Link to="/studio/profile" onClick={closeAllMenus} className="flex items-center gap-3 px-5 py-3.5 rounded-2xl text-xs font-black text-white/60 hover:text-white hover:bg-white/5 transition-all"><User size={18} /> الملف الشخصي</Link>
      ...
      >
          <div className="flex items-center justify-between mb-12">
             <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-2xl italic">م</div>
                <span className="text-3xl font-black text-white italic">مداد</span>
             </div>
             <button onClick={closeAllMenus} aria-label="إغلاق" className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white"><X size={24} /></button>
          </div>
          <div className="space-y-3 flex-1 text-right" onClick={closeAllMenus}>
              {user && (
                <Link to="/studio/posts/new" className="flex items-center gap-4 p-4 rounded-2xl bg-purple-600 text-sm font-black text-white shadow-lg shadow-purple-600/20 mb-4">
                  <PenSquare size={20} /> أنتج مقالاً جديداً
                </Link>
              )}
              <div className="space-y-1">
                {navLinks.map(link => (
                  <Link key={link.to} to={link.to} className="flex items-center gap-4 p-4 rounded-2xl text-white/60 hover:text-white hover:bg-white/5 text-sm font-black transition-all">
              ...
              {!user ? (
                <div className="pt-6 space-y-3">
                  <Link to="/login" className="flex items-center justify-center w-full py-4 rounded-2xl bg-white text-black font-black text-sm shadow-xl">تسجيل الدخول</Link>
                  <Link to="/register" className="flex items-center justify-center w-full py-4 rounded-2xl border border-white/10 text-white font-black text-sm">إنشاء حساب</Link>
                </div>
              ) : (
                <div className="pt-6 space-y-2">
                   <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-4 mb-2">لوحة التحكم</p>
                   <Link to="/studio" className="flex items-center gap-4 p-4 rounded-2xl text-white/60 hover:text-white hover:bg-white/5 text-sm font-black transition-all">
                      <LayoutDashboard size={20} /> استوديو المبدعين
                   </Link>
                   <button onClick={handleSignOut} className="flex items-center gap-4 p-4 rounded-2xl text-red-400 hover:bg-red-500/10 text-sm font-black transition-all w-full text-right">
