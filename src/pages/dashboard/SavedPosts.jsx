...
import { Bookmark, Sparkles, Clock, ArrowLeft, Trash2, BookOpen, User as UserIcon } from 'lucide-react'
...
export default function SavedPosts() {
  ...
  async function removeSave(saveId) {
    try {
      const { error } = await supabase.from('saved_posts').delete().eq('id', saveId)
      if (error) throw error
      setSavedPosts(prev => prev.filter(p => p.id !== saveId))
      toast.success('تمت إزالة المقال من المحفوظات')
    } catch (err) { 
      toast.error('فشل في إزالة الحفظ: ' + err.message)
    }
  }

  if (loading) return <div className="flex justify-center py-40"><LoadingSpinner className="w-16 h-16 text-purple-600" /></div>
    ...
    <div className="max-w-6xl mx-auto pb-20" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-16">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-white/30 mt-4 font-black uppercase tracking-[0.3em] text-[10px]">مجموعتك المختارة من أفضل المقالات</p>
        </motion.div>
        ...
        >
          ...
          <div>
            <p className="text-2xl md:text-4xl font-black tabular-nums leading-none text-white">{savedPosts.length}</p>
            <p className="text-[10px] font-black text-white/20 uppercase tracking-widest mt-2">مقال محفوظ</p>
          </div>
        </motion.div>
      </div>
      ...
      {savedPosts.length === 0 ? (
        <div className="text-center py-40 bg-[#0d0d0d]/40 backdrop-blur-3xl border border-dashed border-white/10 rounded-[4rem] px-10 shadow-2xl">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-white mb-4 italic">مكتبتك فارغة حالياً</h2>
          <p className="text-white/20 font-bold mb-12 max-w-sm mx-auto text-sm md:text-lg">ابدأ باستكشاف المقالات وقم بحفظ ما يعجبك للعودة إليه لاحقاً.</p>
          <Link to="/" className="inline-flex items-center gap-4 bg-purple-600 text-white px-10 py-5 rounded-3xl font-black text-lg shadow-xl shadow-purple-600/20 hover:bg-purple-500 transition-all">
                ...
                >
                  <div className="w-full md:w-80 aspect-[16/10] rounded-[2.5rem] overflow-hidden bg-white/5 shrink-0 shadow-2xl group-hover:shadow-purple-600/10 transition-all duration-700 border border-white/5">
                    {post.cover_image_url ? (
                      <OptimizedImage src={post.cover_image_url} alt={post.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-white/5"><Sparkles size={48} /></div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 space-y-6 text-right relative z-10 w-full">
                    <div className="flex items-center gap-4 flex-wrap">
                      <span className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-2">
                        <Clock size={12} className="text-purple-500" /> {formatDate(post.published_at)}
                      </span>
                    ...
                    <p className="text-white/20 text-xs md:text-lg font-bold line-clamp-2 leading-relaxed hidden sm:block">
                      {post.excerpt || 'هذا المقال لا يحتوي على وصف مختصر، اضغط للمزيد من التفاصيل...'}
                    </p>
                    ...
                    <div className="flex items-center gap-4 pt-6">
                      ...
                      >
                        قراءة المقال <ArrowLeft size={18} className="rotate-180" />
                      </Link>
                      <button 
