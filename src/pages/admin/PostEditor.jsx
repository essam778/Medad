...
export default function PostEditor() {
  ...
  async function handleMediumImport() {
    if (!mediumUrl.trim()) return toast.error("يرجى إدخال رابط Medium أولاً");
    setIsImporting(true);
    try {
      const { data, error } = await supabase.functions.invoke("medium-import", {
        body: { url: mediumUrl },
      });
      if (error) throw error;
      ...
      if (data?.success) {
        ...
        toast.success("تم استيراد المقال من Medium بنجاح!");
  ...
  async function handleSave(statusOverride) {
    if (!form.title.trim() || !form.content.trim())
      return toast.error("يرجى ملء العنوان والمحتوى");
    setSaving(true);
    try {
      const sanitizedContent = DOMPurify.sanitize(form.content);
      const finalTags = form.series
        ? [...form.tags, `series:${form.series}`]
        : form.tags;
      ...
      if (
        ...
        try {
          ...
          if (rpcError) {
            console.error(
              "RPC Points failed, trying manual fallback:",
              rpcError,
            );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-8 lg:p-12 font-['Inter',sans-serif] selection:bg-purple-600/30">
      <Helmet>
        <title>{id ? "تعديل المقال" : "مقال جديد"} | مداد</title>
      </Helmet>
      ...
      <header className="max-w-7xl mx-auto mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <Link
            to="/admin/posts"
            className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white/40 hover:text-white transition-all border border-white/5"
          >
            <ArrowRight size={24} />
          </Link>
          <h1 className="text-2xl md:text-3xl font-black italic">
            {id ? "تحرير المقال" : "مقال جديد"}
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setYoutubeModal(true)}
            className="px-6 py-3 bg-red-600/10 text-red-500 rounded-xl text-[10px] font-black hover:bg-red-600 hover:text-white transition-all flex items-center gap-2 border border-red-500/20"
          >
            <Youtube size={16} /> ذكاء يوتيوب
          </button>
          <button
            onClick={() => setMediumModal(true)}
            className="px-6 py-3 bg-emerald-600/10 text-emerald-500 rounded-xl text-[10px] font-black hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-2 border border-emerald-500/20"
          >
            <FileText size={16} /> استيراد من Medium
          </button>
          <button
            onClick={() => handleSave("draft")}
            disabled={saving}
            className="px-4 py-3 bg-white/5 rounded-xl text-[10px] font-black hover:text-white transition-all"
          >
            مسودة
          </button>
          <button
            onClick={() =>
              handleSave(form.scheduled_for ? "scheduled" : "published")
            }
            disabled={saving}
            className="px-8 py-3 bg-purple-600 text-white rounded-xl text-[10px] font-black hover:bg-purple-500 transition-all active:scale-95"
          >
            {saving ? (
      ...
      <main className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-8 text-right">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl p-6 md:p-8 shadow-2xl">
            <textarea
              ...
              rows={2}
              className="w-full text-xl md:text-3xl font-black border-none outline-none bg-transparent text-white resize-none italic text-right"
            />
            <div className="flex items-center justify-end gap-2 text-[8px] bg-white/5 w-fit mr-auto px-3 py-1.5 rounded-lg">
              <code className="text-purple-400">
                {form.slug || "automatic-slug"}
              </code>
          ...
          <section className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center justify-end gap-2 mb-4">
              <h3 className="font-black text-[10px] text-white/20 uppercase">
                الغلاف
              </h3>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between px-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-purple-600/10 text-purple-400 rounded-xl text-[10px] font-black hover:bg-purple-600 hover:text-white transition-all"
              >
                {isSummarizing ? (
        ...
        <aside className="space-y-8">
          <section className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl p-6 shadow-2xl">
            <h3 className="font-black text-[10px] text-white/20 mb-4 uppercase flex items-center justify-end gap-2">
              السلسلة <LayoutDashboard size={14} />
            </h3>
            <input
              value={form.series}
              onChange={(e) => set("series", e.target.value)}
              placeholder="اسم السلسلة..."
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs font-black text-white outline-none focus:border-purple-600 text-right"
            />
          </section>

          <section className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl p-6 shadow-2xl">
            <h3 className="font-black text-[10px] text-white/20 mb-4 uppercase flex items-center justify-end gap-2">
              التصنيفات <TagIcon size={14} />
            </h3>
            <div className="flex flex-wrap gap-2 mb-4 justify-end">
              {form.tags.map((tag) => (
                <span
                  key={tag}
                  className="flex items-center gap-2 bg-purple-600/10 text-purple-400 text-[9px] font-black px-3 py-1.5 rounded-lg border border-purple-500/20"
                >
                  #{tag}
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs font-black text-white text-right"
            />
          </section>

          <section className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl p-6 shadow-2xl space-y-6">
            <h3 className="font-black text-[10px] text-white/20 uppercase flex items-center justify-end gap-2">
              الجدولة <Clock size={14} />
            </h3>
            <input
              type="datetime-local"
              value={form.scheduled_for}
              onChange={(e) => set("scheduled_for", e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-[10px] font-black text-white [color-scheme:dark]"
            />
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
              <button
                onClick={() =>
                  set("comments_disabled", !form.comments_disabled)
                }
                className={`w-10 h-6 rounded-full transition-all relative p-1 ${form.comments_disabled ? "bg-purple-600" : "bg-white/10"}`}
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white transition-all ${form.comments_disabled ? "mr-4" : "mr-0"}`}
                />
              </button>
          ...
          <section className="bg-red-600/5 border border-red-600/10 rounded-2xl p-6 shadow-2xl space-y-4">
            <h3 className="font-black text-[10px] text-red-400 uppercase flex items-center justify-end gap-2">
              منطقة الخطر <X size={14} />
            </h3>
            <button
              onClick={() => {
                setNotice({
                  ...
                  title: "تأكيد التصفير",
                  message:
                    "هل أنت متأكد من مسح كافة محتويات المقال؟ لا يمكن التراجع عن هذه العملية.",
                  variant: "warning",
                  onAction: () => {
                    setForm(INITIAL);
                    localStorage.removeItem("midad_post_draft");
        ...
        {youtubeModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isSummarizing && setYoutubeModal(false)}
            ...
            <motion.div
              ...
              className="bg-[#0d0d0d] border border-white/10 rounded-[2rem] p-8 max-w-lg w-full relative z-10 shadow-2xl"
            >
              <h2 className="text-xl font-black mb-6 flex items-center justify-between">
                ذكاء يوتيوب{" "}
                <button onClick={() => setYoutubeModal(false)}>
                  <X size={20} />
                </button>
              </h2>
              <div className="space-y-6">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-white text-sm"
                />
                <textarea
                  value={manualText}
                  onChange={(e) => setManualText(e.target.value)}
                  placeholder="أو الصق النص هنا..."
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-6 text-white text-sm h-32 resize-none"
                />
                <button
                  disabled={isSummarizing || (!youtubeUrl && !manualText)}
                  onClick={
                    manualText ? handleManualSummarize : handleYoutubeSummarize
                  }
                  className="w-full bg-red-600 text-white py-5 rounded-xl font-black flex items-center justify-center gap-3 hover:bg-red-500 disabled:opacity-50"
                >
                  {isSummarizing ? (
        ...
        {mediumModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isImporting && setMediumModal(false)}
              className="absolute inset-0 bg-black/80 backdrop-blur-xl"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.9 }}
            ...
            >
              <h2 className="text-xl font-black mb-6 flex items-center justify-between">
                استيراد من Medium{" "}
                <button onClick={() => setMediumModal(false)}>
                  <X size={20} />
                </button>
              </h2>
              <div className="space-y-6">
                <input
                  ...
                  placeholder="رابط مقال Medium..."
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-white text-sm"
                />
                <button
                  disabled={isImporting || !mediumUrl.trim()}
                  onClick={handleMediumImport}
                  className="w-full bg-emerald-600 text-white py-5 rounded-xl font-black flex items-center justify-center gap-3 hover:bg-emerald-500 disabled:opacity-50"
                >
                  {isImporting ? (
                    <Loader2 className="animate-spin" size={20} />
                  ) : (
