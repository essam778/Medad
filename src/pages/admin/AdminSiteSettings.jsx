...
export default function AdminSiteSettings() {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    try {
      const { error } = await PostService.upsertChannelSettings(
        user.id,
        settings,
      );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
        ...
        >
          ...
          <p className="text-white/30 mt-4 font-black uppercase tracking-[0.3em] text-[10px]">
            تحكم في هوية قناتك وظهورها للجمهور
          </p>
        </motion.div>
            ...
            <div className="flex-1 space-y-8 w-full">
              ...
              <div className="flex flex-col gap-4">
                <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-2 flex items-center gap-2">
                  <AtSign size={14} className="text-purple-500" /> رابط القناة
                  المخصص
                </label>
                <div className="relative">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 text-white/20 font-black text-sm italic">
                    midad.me/c/
                  </span>
                  <input
                    type="text"
      ...
      <div className="mt-16 bg-red-500/5 border border-red-500/20 rounded-[3rem] p-10 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          <p className="text-red-400/60 text-sm font-bold mb-8 max-w-2xl leading-relaxed">
            تحذير: حذف القناة سيؤدي إلى مسح الهوية البصرية (الشعار) والرابط
            المخصص فوراً. لن يتم حذف المقالات المنشورة، ولكنها ستفقد ارتباطها
            بهوية القناة. هذا الإجراء غير قابل للتراجع.
          </p>
          <button
          ...
          >
            ...
            >
              ...
              <p className="text-base font-bold text-white/40 mb-8 leading-relaxed text-right">
                سيتم مسح بيانات القناة بالكامل. لتأكيد رغبتك، يرجى كتابة كلمة{" "}
                <strong className="text-red-500">"حذف"</strong> في الحقل أدناه:
              </p>
              <input
                type="text"
                value={deleteConfirm}
