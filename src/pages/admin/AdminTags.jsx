...
import { 
  ...
  Check, X, Loader2, Hash, Sparkles, FileText
} from 'lucide-react'
...
export default function AdminTags() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState({ open: false, tagId: null, tagName: '' })
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      setTags(prev => [...prev, { ...data, usage_count: 0 }].sort((a, b) => a.name.localeCompare(b.name)))
      setNewTag('')
      toast.success('تم إضافة التصنيف بنجاح')
    } catch (err) { toast.error('فشل الإضافة: ' + err.message) } finally { setAdding(false) }
  }
  ...
  async function handleUpdate(id) {
    ...
    try {
      ...
      if (error) throw error
      setTags(prev => prev.map(t => t.id === id ? { ...t, name: editValue.trim().toLowerCase() } : t))
      setEditingId(null)
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-0" dir="rtl">
      <ConfirmModal 
        ...
        onCancel={() => setDeleteConfirm({ open: false, tagId: null, tagName: '' })}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-16">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-white/30 mt-4 font-black uppercase tracking-[0.3em] text-[10px]">نظم مقالات المنصة بتصنيفات واضحة وجذابة</p>
        </motion.div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full lg:w-auto">
          {[
            { label: 'إجمالي التصنيفات', value: tags.length, icon: Tag, color: 'text-purple-400', bg: 'bg-purple-600/10' },
            { label: 'المقالات المصنفة', value: tags.reduce((acc, t) => acc + (t.usage_count || 0), 0), icon: FileText, color: 'text-blue-400', bg: 'bg-blue-600/10' },
            { label: 'التصنيف الأكثر انتشاراً', value: tags.length > 0 ? [...tags].sort((a,b) => b.usage_count - a.usage_count)[0].name : '-', icon: Sparkles, color: 'text-pink-400', bg: 'bg-pink-600/10' },
          ].map((stat, i) => (
            <motion.div 
            ...
            >
              <div className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center border border-white/5`}>
                <stat.icon size={20} />
              </div>
              <div className="text-right">
                <p className="text-xl md:text-2xl font-black tabular-nums leading-none text-white italic">{typeof stat.value === 'number' ? stat.value.toLocaleString('ar-EG') : stat.value}</p>
                <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em] mt-2">{stat.label}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <form onSubmit={handleAdd} className="mb-16">
        <div className="relative group">
          <div className="absolute right-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-purple-500 transition-colors"><Tag size={24} /></div>
          <input 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        <AnimatePresence>
          {tags.map((tag, i) => (
            <motion.div 
              key={tag.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: i * 0.05 }}
              ...
              {editingId === tag.id ? (
                <div className="space-y-4 relative z-10">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => handleUpdate(tag.id)} className="flex-1 bg-purple-600 text-white py-4 rounded-2xl font-black text-xs hover:bg-purple-500 transition-all flex items-center justify-center gap-2"><Check size={16} /> حفظ</button>
                    <button onClick={() => setEditingId(null)} className="flex-1 bg-white/5 text-white/40 py-4 rounded-2xl font-black text-xs hover:bg-white/10 transition-all flex items-center justify-center gap-2 border border-white/10"><X size={16} /> إلغاء</button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-4 md:gap-6">
                    ...
                    <div>
                      <p className="font-black text-lg md:text-2xl text-white group-hover:text-purple-400 transition-colors italic">{tag.name}</p>
                      <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mt-1.5">{tag.usage_count || 0} مقال مرتبط</p>
                    </div>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                    <button onClick={() => { setEditingId(tag.id); setEditValue(tag.name); }} className="w-10 h-10 bg-white/5 text-white/20 hover:text-white hover:bg-white/10 rounded-xl flex items-center justify-center transition-all border border-white/5"><Edit2 size={16} /></button>
                    <button onClick={() => setDeleteConfirm({ open: true, tagId: tag.id, tagName: tag.name })} className="w-10 h-10 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white rounded-xl flex items-center justify-center transition-all border border-red-500/10"><Trash2 size={16} /></button>
                  </div>
                </div>
              )}
            </motion.div>
