...
import {
  FolderOpen, Plus, Trash2, Edit3, ChevronDown, ChevronUp,
  FileText, Loader2, Check, X, Save, FolderPlus, ListVideo, Layers, Sparkles
} from 'lucide-react'
...
export default function AdminGroups() {
  ...
  async function fetchGroups() {
    ...
    try {
      const { data, error } = await supabase
        .from('collections')
        .select(`*, collection_posts(post_id, posts(id, title, slug, status, cover_image_url))`)
        .eq('author_id', user.id)
        .order('created_at', { ascending: false })
      if (error) throw error
      setGroups(data || [])
    } catch (err) {
      toast.error('خطأ في تحميل القوائم: ' + err.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await supabase.from('collection_posts').delete().eq('collection_id', deleteConfirm.groupId)
      const { error } = await supabase.from('collections').delete()
        .eq('id', deleteConfirm.groupId).eq('author_id', user.id)
      if (error) throw error
  ...
  async function removePostFromGroup(groupId, postId) {
    try {
      await supabase.from('collection_posts')
        .delete()
  ...
  const groupPostIds = (group) => (group.collection_posts || []).map(cp => cp.post_id)
  const availablePosts = (group) => posts.filter(p => !groupPostIds(group).includes(p.id))
    ...
    <div className="max-w-6xl mx-auto pb-20 px-0" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10 px-4 md:px-0">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="text-right">
          ...
          <p className="text-white/30 mt-2 font-black uppercase tracking-[0.3em] text-[8px] pr-2 text-right">نظّم مقالاتك في سلاسل وقوائم تشغيل احترافية</p>
        </motion.div>
        
        <button
          onClick={() => { setCreateModal(true); setForm({ name: '', description: '' }) }}
          ...
          إنشاء سلسلة جديدة <FolderPlus size={20} className="group-hover:rotate-12 transition-transform" />
        </button>
      </div>
      ...
      {groups.length === 0 ? (
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-black text-white mb-4 italic text-center">لا توجد سلاسل بعد</h2>
          <p className="text-white/20 font-bold mb-10 max-w-sm mx-auto text-sm md:text-lg text-center">ابدأ بإنشاء أول سلسلة لتنظيم مقالاتك بشكل متصل وجذاب.</p>
          <button
            onClick={() => { setCreateModal(true); setForm({ name: '', description: '' }) }}
            ...
            >
              <div className="flex flex-col md:flex-row-reverse items-center gap-5 p-5 md:p-6">
                ...
                <div className="flex-1 min-w-0 text-right">
                  <h2 className="font-black text-base md:text-xl text-white truncate italic group-hover/card:text-purple-400 transition-colors">{group.name}</h2>
                ...
                <div className="flex items-center gap-3 shrink-0 flex-row-reverse">
                  <button
                    onClick={() => { setEditModal(group); setForm({ name: group.name, description: group.description || '' }) }}
                  ...
                  <button
                    onClick={() => setDeleteConfirm({ open: true, groupId: group.id, groupName: group.name })}
                  ...
                  >
                    <div className="p-4 md:p-6 space-y-3">
                      {(group.collection_posts || []).length === 0 ? (
                        <p className="text-center text-white/20 text-[10px] font-bold py-6 italic">لا توجد مقالات حالياً</p>
                      ) : (
                        (group.collection_posts || []).map(cp => cp.posts && (
                          <div key={cp.post_id} className="flex items-center flex-row-reverse gap-4 p-3 bg-[#050505]/40 rounded-xl border border-white/5 group/item">
                            <div className="w-12 h-9 rounded-lg overflow-hidden bg-white/5 border border-white/5 shrink-0">
                                {cp.posts.cover_image_url && <OptimizedImage src={cp.posts.cover_image_url} alt="" className="w-full h-full object-cover" />}
                            </div>
                            <div className="flex-1 min-w-0 text-right">
                              <p className="font-black text-xs md:text-sm text-white/80 truncate">{cp.posts.title}</p>
                            </div>
                            <button
                              onClick={() => removePostFromGroup(group.id, cp.post_id)}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6" dir="rtl">
            ...
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
              ...
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => { setCreateModal(false); setEditModal(null) }} className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl text-white/40 hover:text-white border border-white/5">
                  <X size={24} />
                </button>
              </div>
              <form onSubmit={editModal ? handleEdit : handleCreate} className="space-y-8 text-right">
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-2 text-right block">اسم السلسلة *</label>
                  <input
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] px-2 text-right block">الوصف (اختياري)</label>
                  <textarea
                ...
                <button type="submit" disabled={saving}
                  ...
                  {saving ? 'جاري الحفظ...' : editModal ? 'حفظ التعديلات' : 'إنشاء السلسلة الآن'}
                </button>
              </form>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6" dir="rtl">
            ...
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
              ...
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => setAddPostModal(null)} className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl text-white/40 hover:text-white border border-white/5"><X size={24} /></button>
              </div>
              <div className="overflow-y-auto flex-1 space-y-4 custom-scrollbar pr-4">
                {availablePosts(addPostModal).length === 0 ? (
                  <div className="text-center py-20 text-white/20 font-black italic text-center">لا توجد مقالات متاحة</div>
                ) : (
                  availablePosts(addPostModal).map(post => (
                    <button
                      key={post.id}
                      onClick={() => { addPostToGroup(addPostModal.id, post.id); setAddPostModal(null) }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 text-right">
                        <p className="font-black text-lg text-white truncate">{post.title}</p>
                      </div>
