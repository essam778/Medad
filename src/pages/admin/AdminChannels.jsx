...
export default function AdminChannels() {
  ...
  async function toggleBanAuthor(authorId, isBanned) {
    try {
      ...
      toast.success(
        isBanned ? "تم رفع الحظر عن المبدع" : "تم حظر المبدع نهائياً",
      );
      queryClient.invalidateQueries({ queryKey: ["admin", "channels"] });
    } catch (err) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10 px-4 md:px-0">
        ...
        >
          ...
          <p className="text-white/30 mt-2 font-black uppercase tracking-[0.3em] text-[8px]">
            أدر جميع القنوات المبدعة على المنصة ({count})
          </p>
        </motion.div>
        ...
        <div className="relative w-full lg:w-[400px] group">
          ...
          <input
            type="text"
            placeholder="ابحث باسم القناة أو المبدع..."
        ...
        {[
          ...
          },
          {
            ...
            value: channels.reduce(
              (acc, ch) => acc + (ch.followersCount || 0),
              0,
            ),
            icon: Users,
            color: "text-blue-400",
          ...
          },
          {
            ...
            value: channels.reduce((acc, ch) => acc + (ch.postsCount || 0), 0),
            icon: FileText,
              ...
              {channels.map((ch, i) => (
                ...
                >
                  <div className="flex items-center gap-5 flex-1 min-w-0 relative z-10">
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden bg-white/5 border border-white/10 shadow-xl shrink-0">
                      {ch.logo_url ? (
                        ...
                        <div className="w-full h-full bg-purple-600 flex items-center justify-center font-black text-xl italic text-white">
                          {ch.site_name?.[0]}
                        </div>
                      )}
                    </div>
                    <div className="min-w-0 flex-1 text-right">
                      <div className="flex items-center gap-3 flex-wrap mb-1">
                        <h3 className="font-black text-base md:text-xl text-white truncate italic">
                          {ch.site_name}
                        </h3>
                        {ch.profiles?.is_banned && (
                          <span className="text-[8px] font-black uppercase tracking-widest bg-red-500 text-white px-3 py-0.5 rounded-full">
                            محظور
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] md:text-xs text-white/20 font-black italic truncate mb-2">
                        midad.me/c/{ch.channel_slug}
                      </p>
                  ...
                  <div className="flex items-center justify-center lg:justify-end gap-3 shrink-0 relative z-10">
                    ...
                    <button
                      onClick={() =>
                        toggleBanAuthor(ch.author_id, ch.profiles?.is_banned)
                      }
                    ...
                    <button
                      onClick={() =>
                        setDeleteConfirm({ open: true, channel: ch })
                      }
