...
import { 
  Users, UserIcon, Search, Trash2, Crown, PenSquare, 
  Loader2, Sparkles, ChevronRight, ChevronLeft, Filter
} from 'lucide-react'
...
const ROLES = [
  { value: 'admin', label: 'مدير', icon: Crown, color: 'text-purple-400', bg: 'bg-purple-600/10', border: 'border-purple-500/20' },
  { value: 'author', label: 'كاتب', icon: PenSquare, color: 'text-blue-400', bg: 'bg-blue-600/10', border: 'border-blue-500/20' },
  { value: 'reader', label: 'قارئ', icon: UserIcon, color: 'text-white/20', bg: 'bg-white/5', border: 'border-white/5' },
]
...
export default function AdminUsers() {
  ...
  const { data, isLoading } = useAdminUsers({ page, search: searchTerm, role: roleFilter })
  ...
  async function updateRole(userId, newRole) {
    ...
    } catch (err) { toast.error('خطأ في تحديث الرتبة: ' + err.message) }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 px-0" dir="rtl">
      <ConfirmModal 
        ...
        confirmLabel={deleteUserMutation.isPending ? 'جاري الحذف...' : 'حذف نهائي'}
        cancelLabel="تراجع"
        variant="danger"
      />
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-white/30 mt-2 font-black uppercase tracking-[0.3em] text-[8px]">تحكم في رتب الأعضاء وصلاحياتهم ({count})</p>
        </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 px-4 md:px-0">
         {[
           { label: 'إجمالي الأعضاء', value: count, icon: Users, color: 'text-white', bg: 'bg-white/5' },
           { label: 'فريق الإدارة', value: users.filter(u => u.role === 'admin').length, icon: Crown, color: 'text-purple-400', bg: 'bg-purple-600/10' },
           { label: 'صناع المحتوى', value: users.filter(u => u.role === 'author').length, icon: PenSquare, color: 'text-blue-400', bg: 'bg-blue-600/10' },
         ].map((stat, i) => (
           <motion.div 
      ...
      <div className="flex flex-wrap items-center gap-4 mb-10 bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 p-6 rounded-2xl shadow-xl">
        ...
        <div className="flex flex-wrap gap-2">
          {[['', 'الكل'], ...ROLES.map(r => [r.value, r.label])].map(([val, lbl]) => (
            <button 
              key={val} onClick={() => { setRoleFilter(val); setPage(0) }}
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black transition-all border ${
                roleFilter === val ? 'bg-purple-600 text-white border-purple-500 shadow-lg' : 'text-white/30 hover:text-white border-white/5'
              }`}
            >
              {lbl}
            </button>
      ...
      <div className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
        {isLoading ? (
          <div className="py-40 flex flex-col items-center gap-6">
            ...
            <p className="text-white/20 font-black text-xs uppercase tracking-widest italic">جاري جلب قائمة الأعضاء...</p>
          </div>
        ) : users.length === 0 ? (
          <div className="text-center py-40">
            <Sparkles size={64} className="mx-auto mb-8 text-white/5" />
            <p className="text-white/10 font-black italic text-2xl uppercase tracking-widest">لا يوجد أعضاء يطابقون بحثك</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-right">
                <thead>
                  <tr className="bg-white/[0.01] border-b border-white/5 text-white/20">
                    <th className="px-6 py-6 text-[9px] font-black uppercase tracking-[0.2em]">العضو</th>
                    <th className="px-6 py-6 text-[9px] font-black uppercase tracking-[0.2em] hidden lg:table-cell text-center">الانضمام</th>
                    <th className="px-6 py-6 text-[9px] font-black uppercase tracking-[0.2em] text-center">الرتبة</th>
                    <th className="px-6 py-6 text-[9px] font-black uppercase tracking-[0.2em] text-left">الإجراءات</th>
                  </tr>
                </thead>
                <tbody className="divide-y border-white/5">
                  {users.map((u, i) => (
                    <motion.tr 
                      key={u.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/5 border border-white/5 shadow-xl overflow-hidden shrink-0">
                            {u.avatar_url ? <OptimizedImage src={u.avatar_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform" /> : <div className="w-full h-full flex items-center justify-center text-white/10 font-black text-lg italic">{u.full_name?.[0]}</div>}
                          </div>
                          <div className="min-w-0">
                            <p className="font-black text-xs md:text-sm text-white mb-1 truncate italic">{u.full_name}</p>
                            <p className="text-[8px] text-white/20 font-black uppercase">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        {ROLES.filter(r => r.value === u.role).map(r => (
                          <div key={r.value} className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[8px] font-black uppercase border ${r.bg} ${r.color} ${r.border}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-3">
                          <div className="flex items-center gap-1.5 bg-white/5 p-1 rounded-xl border border-white/5">
                            {ROLES.map(role => (
                              <button
                                key={role.value} onClick={() => updateRole(u.id, role.value)}
                                disabled={updateRoleMutation.isPending || u.role === role.value}
                          ...
                          <button
                            onClick={() => setDeleteConfirm({ open: true, userId: u.id, userName: u.full_name })}
                            disabled={deleteUserMutation.isPending}
                            className="p-2.5 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all border border-red-500/10"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
