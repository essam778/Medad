...
export default function AdminPosts() {
  ...
  const { data, isLoading } = useAdminPosts({ status, page, authorId })
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-16">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-white/30 mt-4 font-black uppercase tracking-[0.3em] text-[10px]">
            {total.toLocaleString('ar-EG')} مقال مسجل في المنصة
          </p>
        </motion.div>
      ...
      <div className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] p-4 md:p-8 mb-12 shadow-2xl">
      ...
      <div className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/10 rounded-[3rem] md:rounded-[4rem] overflow-hidden shadow-2xl">
        {isLoading ? (
          <div className="py-40 flex flex-col items-center gap-6">
            ...
            <p className="text-white/20 font-black text-xs uppercase tracking-widest">جاري جلب المحتوى...</p>
          </div>
        ) : posts.length === 0 ? (
          <div className="text-center py-40 px-10">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-white italic">لا توجد مقالات هنا</h2>
            <p className="text-white/20 font-bold mt-4 text-sm md:text-lg">ابدأ بكتابة مقالك الأول الآن وانطلق للعالمية.</p>
          </div>
        ) : (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="min-w-0">
                             <p className="font-black text-xs md:text-sm text-white/80 truncate group-hover:text-purple-400 transition-colors mb-1">{post.title}</p>
      ...
      {totalPages > 1 && (
        <div className="mt-16 flex items-center justify-center gap-8">
          ...
          <div className="bg-purple-600 text-white px-10 py-4 rounded-[2rem] font-black tabular-nums shadow-xl shadow-purple-600/20 border border-purple-400/20">
