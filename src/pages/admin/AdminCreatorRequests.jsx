...
import { 
  Check, X, Loader2, User, Mail, Clock, 
  MessageSquare, ExternalLink, ShieldCheck, 
  Sparkles, Bell, ArrowLeft, Trash2, Zap
} from 'lucide-react'
...
export default function AdminCreatorRequests() {
  ...
  async function handleAction(userId, requestId, action) {
    ...
    try {
      if (action === 'approve') {
        const { error } = await ProfileService.approveCreatorRequest(requestId, userId)
        if (error) throw error
        toast.success('تمت الموافقة على انضمام المبدع')
      } else {
        const { error } = await ProfileService.rejectCreatorRequest(requestId)
        if (error) throw error
    ...
    <div className="max-w-6xl mx-auto pb-20 px-0" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
        ...
        {requests.length > 0 && (
          ...
          >
            ...
            <div>
              <p className="text-xl md:text-2xl font-black tabular-nums leading-none text-white">{requests.length}</p>
              <p className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] mt-1.5">تنبيه نشط</p>
            </div>
          </motion.div>
        )}
      </div>
      ...
      {requests.length === 0 ? (
        <div className="text-center py-40 bg-[#0d0d0d]/40 backdrop-blur-3xl border border-dashed border-white/10 rounded-[4rem] px-10 shadow-2xl">
           ...
           <h2 className="text-2xl md:text-3xl font-black text-white mb-4 italic">الرادار نظيف تماماً</h2>
           <p className="text-white/20 font-bold mt-3 max-w-sm mx-auto leading-relaxed text-sm md:text-lg">لقد قمت بمراجعة كافة طلبات الانضمام بنجاح. لا توجد مهام معلقة في الوقت الحالي.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <AnimatePresence mode="popLayout">
            {requests.map((req, i) => (
              <motion.div 
                key={req.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.05 }}
              ...
              >
                <div className="flex items-center gap-4 mb-6 relative z-10">
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-white/5 border border-white/5 shadow-xl overflow-hidden shrink-0">
                    {req.profiles?.avatar_url ? <OptimizedImage src={req.profiles.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white/10 font-black text-lg italic">{req.profiles?.full_name?.[0]}</div>}
                  </div>
                  <div className="min-w-0 flex-1">
                    <h3 className="font-black text-sm md:text-lg text-white truncate italic">{req.profiles?.full_name}</h3>
                    <p className="text-[8px] text-white/20 font-black uppercase tracking-widest mt-1">{req.profiles?.email}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 relative z-10">
                  <button 
                    disabled={processingId === req.id} onClick={() => handleAction(req.user_id, req.id, 'approve')} 
                  ...
                  <button 
                    disabled={processingId === req.id} onClick={() => handleAction(req.user_id, req.id, 'reject')} 
