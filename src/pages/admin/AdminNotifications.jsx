...
export default function AdminNotifications() {
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "فشل في الإرسال"));
    } finally {
      setSending(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "فشل الحذف"));
    } finally {
      setDeletingId(null);
    }
  }

  if (loadingHistory && recentNotifs.length === 0)
    return (
    ...
    <div className="max-w-7xl mx-auto pb-20 px-0" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-16 px-4 md:px-0">
        ...
        >
          ...
          <p className="text-white/30 mt-4 font-black uppercase tracking-[0.3em] text-[10px]">
            أرسل إشعارات جماعية أو مخصصة لمجتمع مداد
          </p>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 px-4 md:px-0">
        ...
        <div className="lg:col-span-7">
          <form
            onSubmit={handleSend}
            className="bg-[#0d0d0d]/40 backdrop-blur-3xl border border-white/10 rounded-[3rem] p-8 md:p-12 shadow-2xl space-y-10"
          >
            ...
            <div className="space-y-6">
              <label className="block text-[10px] font-black text-white/20 uppercase tracking-[0.3em] px-2">
                الجمهور المستهدف
              </label>
              <div className="grid grid-cols-2 gap-4">
                {AUDIENCE_OPTIONS.map((opt) => (
                  ...
                  >
                    <opt.icon
                      size={20}
                      className={`shrink-0 transition-colors ${form.audience === opt.id ? "text-purple-400" : ""}`}
                    />
                    ...
                    {form.audience === opt.id && (
                      <motion.div
                        layoutId="aud-glow"
                        className="absolute inset-0 bg-purple-600/5 blur-xl -z-10"
                      />
                    )}
                  </button>
                ))}
            ...
            <AnimatePresence>
              {form.audience === "specific" && (
                ...
                >
                  <label className="block text-[10px] font-black text-white/20 uppercase tracking-[0.3em] px-2">
                    اختر المستخدم المحدد
                  </label>
                  <div className="relative group">
                    <Search
                      className="absolute right-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-purple-500 transition-all"
                      size={20}
                    />
                    <input
                      type="text"
                    ...
                    {filteredUsers.map((u) => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-black truncate italic">
                            {u.full_name}
                          </p>
                          <p className="text-[10px] opacity-40 truncate uppercase tracking-widest">
                            {u.email}
                          </p>
                        </div>
                      </button>
                ...
                recentNotifs.map((n, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-base font-black text-white truncate italic">
                        {n.title}
                      </p>
                      <p className="text-xs text-white/30 font-bold mt-1 leading-relaxed line-clamp-2 pr-4 italic">
                        "{n.message}"
                      </p>
                      <div className="flex items-center gap-4 mt-4">
                        ...
                        <p className="text-[9px] text-purple-500 font-black uppercase tracking-[0.2em]">
                          {n.metadata?.audience === "all"
                            ? "للجميع"
                            : n.metadata?.audience === "authors"
                              ? "للكُتّاب"
                              : n.metadata?.audience === "readers"
                                ? "للقراء"
                                : "محدد"}
                        </p>
                      </div>
