...
export default function PostPage() {
  ...
  async function fetchPost() {
    ...
    try {
      ...
      const { data: channelData } = await PostService.getAuthorChannel(
        data.author_id,
      );
      ...
      if (user) {
        const { data: likeData } = await PostService.checkLikeStatus(
          data.id,
          user.id,
        );
        setLiked(likeData ? likeData.type : false);
        const { data: saveData } = await PostService.checkSaveStatus(
          data.id,
          user.id,
        );
        setSaved(!!saveData);
        const { data: followData } = await PostService.checkFollowStatus(
          user.id,
          data.author_id,
        );
  ...
  return (
    <div
      className="min-h-screen bg-[#050505] text-white font-arabic pb-32"
      dir="rtl"
    >
      <Helmet>
        <title>
          {post.title} | {authorChannel?.site_name || "مداد"}
        </title>
      </Helmet>
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          ...
          >
            <div className="flex items-center gap-4 mb-6 text-purple-400 font-black text-xs uppercase tracking-widest">
              <span className="bg-purple-600/20 px-4 py-2 rounded-xl border border-purple-500/20">
                # {post.tags?.[0] || "تحليل"}
              </span>
              {post.views > 100 && (
              ...
              <span className="flex items-center gap-2 text-white/80">
                <Clock size={16} /> {readTime} دقائق قراءة
              </span>
            </div>
            <h1 className="text-4xl md:text-7xl font-black italic leading-[1.1] mb-12 drop-shadow-2xl">
              {post.title}
            </h1>
            <div className="flex flex-wrap items-center gap-6">
              {authorChannel && (
                <Link
                  to={`/c/${authorChannel.channel_slug}`}
                  className="flex items-center gap-4 bg-white/5 backdrop-blur-xl border border-white/10 p-2 pl-8 rounded-full hover:bg-white/10 transition-all group"
                >
                  <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-purple-500/30">
                    <OptimizedImage
                      src={getFullImageUrl(authorChannel.logo_url)}
                      alt={authorChannel.site_name}
                      width={48}
                      height={48}
                  ...
                  <div className="text-right">
                    <p className="text-xs font-black group-hover:text-purple-400 transition-colors">
                      {authorChannel.site_name}
                    </p>
              ...
              <button
                ...
                className={`px-10 py-3.5 rounded-full text-xs font-black transition-all ${following ? "bg-white/10 border border-white/10 text-white/80" : "bg-purple-600 text-white hover:bg-purple-500 shadow-xl shadow-purple-600/20"}`}
              >
              ...
              <nav className="flex flex-col gap-4">
                {toc.map((item, i) => (
                  <button
                    ...
                    className={`text-right text-sm font-bold transition-all hover:text-purple-400 ${item.level === "H3" ? "pr-6 text-white/70 text-xs" : "text-white/70"}`}
                  >
                    {item.text}
                  </button>
                ))}
        ...
        <div className="bg-[#0d0d0d]/95 md:bg-[#0d0d0d]/90 md:backdrop-blur-3xl border border-white/10 rounded-full px-4 md:px-10 py-2.5 md:py-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center justify-center gap-2 md:gap-10 pointer-events-auto max-w-[95vw] md:max-w-none">
          ...
          >
            <AnimatePresence>
              {showEmojis && (
                <motion.div
                  ...
                  className="absolute bottom-full mb-6 -right-2 md:right-auto md:left-1/2 md:-translate-x-1/2 flex items-center gap-2 bg-[#050505]/98 md:bg-[#050505]/95 md:backdrop-blur-2xl border border-white/10 p-3 rounded-[2.5rem] shadow-2xl z-[120]"
                >
                  {["like", "love", "haha", "sad", "angry"].map((type) => (
                    <button
                      ...
                      className="flex flex-col items-center gap-1 hover:scale-125 hover:-translate-y-2 transition-transform px-2 group/emoji"
                    >
                      ...
                      <span className="text-[8px] font-black text-white/20 group-hover/emoji:text-purple-400 tabular-nums">
            ...
            <button
              ...
              onClick={() => {
                if (!showEmojis) handleReaction(liked || "like");
              }}
            ...
            >
              ...
              <div className="flex flex-col items-start leading-none">
                <span className="hidden sm:inline text-[9px] mb-0.5">
                  {liked ? "تفاعلت" : "أعجبني"}
                </span>
                <span className="text-[10px] tabular-nums text-white/70">
                  {reactionStats.total.toLocaleString("ar-EG")}
                </span>
              </div>
            </button>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 text-white/70 hover:text-white font-black text-[10px] md:text-sm px-2 py-2 transition-all shrink-0"
          >
            <MessageSquare size={20} />
            <span className="hidden sm:inline">تعليق</span>
            {comments.length > 0 && (
              <span className="bg-purple-600 text-white text-[9px] px-1.5 py-0.5 rounded-lg ml-1">
                {comments.length}
              </span>
            )}
          </button>
          ...
          <button
            onClick={toggleSave}
            className={`flex items-center gap-2 font-black text-[10px] md:text-sm px-2 py-2 rounded-full transition-all ${saved ? "text-purple-400" : "text-white/70 hover:text-white"} shrink-0`}
          >
