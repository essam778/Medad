...
import { Mail, Lock, LogIn, AlertCircle, Loader2, ArrowLeft, Eye, EyeOff } from 'lucide-react'
...
export default function LoginPage() {
  ...
  async function handleOAuthLogin(provider) {
    try {
      ...
      setError(`فشل الدخول عبر ${provider}: ` + err.message)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-4 relative overflow-hidden font-arabic" dir="rtl">
      ...
      >
        <Link to="/" className="text-5xl md:text-6xl font-black text-white tracking-tighter italic drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">مداد</Link>
        <p className="text-purple-300/60 text-xs md:text-sm font-black tracking-[0.4em] uppercase mt-4">مستقبل الحبر الرقمي</p>
      </motion.div>
      ...
      >
        ...
        <div className="relative bg-[#0d0d0d]/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-white mb-2">تسجيل الدخول</h2>
            <p className="text-white/40 text-sm font-bold">أهلاً بك في عالم الإبداع الأدبي المتطور</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center mr-2">
                ...
                <Link to="/forgot-password" size="sm" className="text-[10px] text-white/30 hover:text-white transition-colors font-bold">نسيت كلمة المرور؟</Link>
              </div>
          ...
          <div className="relative my-10">
            ...
            <div className="relative flex justify-center text-[10px] uppercase"><span className="bg-[#0d0d0d] px-4 text-white/20 font-black tracking-widest">أو المتابعة عبر</span></div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => handleOAuthLogin('google')} className="flex items-center justify-center gap-3 bg-white/5 border border-white/10 rounded-2xl py-4 font-bold text-white/80 hover:bg-white/10 transition-all text-xs">
          ...
          <div className="mt-10 text-center">
            <p className="text-white/30 text-sm font-bold">
              ليس لديك حساب؟{' '}
              <Link to="/register" className="text-purple-400 hover:text-purple-300 transition-colors underline-offset-4 hover:underline">أنشئ حساباً جديداً</Link>
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex flex-wrap justify-center gap-6 md:gap-10">
          {['عن مداد', 'الشروط', 'الخصوصية', 'تواصل معنا'].map((link) => (
            <Link key={link} to="#" className="text-white/20 hover:text-white/60 text-xs md:text-sm font-bold transition-colors">{link}</Link>
          ))}
        </div>
        <p className="text-white/10 text-[10px] md:text-xs font-bold tracking-widest">© ٢٠٢٤ مداد - مستقبل الحبر الرقمي</p>
      </motion.footer>
    </div>
  )
}
