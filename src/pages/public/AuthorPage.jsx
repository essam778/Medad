import { getFullImageUrl, getErrorMessage } from "@/lib/utils";
...
export default function AuthorPage() {
  ...
  async function fetchChannelData() {
    ...
    try {
      ...
      const { data: profileData } = await ProfileService.getPublicProfile(
        channelData.author_id,
      );
      setChannel({ ...channelData, profiles: profileData });

      const { data: postsData } = await PostService.getAuthorPosts(
        channelData.author_id,
      );
      setPosts(postsData || []);

      const { data: collectionsData } =
        await PostService.getCollectionsByAuthor(channelData.author_id);
      ...
      setError(getErrorMessage(err, "فشل تحميل بيانات الكاتب"));
    } finally {
      setLoading(false);
    }
  }
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: channel?.site_name || "قناة الكاتب",
      ...
      toast.success("تم نسخ رابط القناة بنجاح!");
    }
  };
  ...
  if (error)
    return (
      <div className="min-h-screen flex flex-col bg-[#050505] items-center justify-center text-center p-6">
        ...
        <Link
          to="/writers"
          className="bg-purple-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-purple-600/20 hover:bg-purple-500 transition-all"
        >
          استكشاف القنوات
        </Link>
      </div>
  ...
  return (
    <div
      className="min-h-screen bg-[#050505] text-white font-arabic pb-24"
      dir="rtl"
    >
      <NoticeModal
        open={notice.open}
            ...
            >
              <div className="w-44 h-44 md:w-64 md:h-64 rounded-[3.5rem] p-1.5 border-2 border-purple-500/20 shadow-[0_50px_100px_rgba(0,0,0,0.5)] bg-[#050505] rotate-3 hover:rotate-0 transition-transform duration-500">
                <div className="w-full h-full rounded-[3rem] overflow-hidden border-4 border-[#0d0d0d] bg-[#0d0d0d]">
                  <OptimizedImage
                    src={getFullImageUrl(channel.logo_url)}
                    alt={channel.site_name}
                    width={256}
                    height={256}
                    fetchpriority="high"
                    loading="eager"
            ...
            <div className="flex-1 text-center md:text-right space-y-6">
              ...
              <p className="text-white/40 text-lg md:text-2xl font-bold max-w-3xl leading-relaxed italic">
                {channel.site_description ||
                  "نشارككم أعمق الأفكار والقصص التي تستحق القراءة."}
              </p>
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-4">
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-center gap-3 bg-white/5 border border-white/5 px-6 py-3 rounded-2xl"
                  >
                ...
                <button
                  onClick={handleShare}
                  className="mr-auto md:mr-0 p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all text-white/40 hover:text-white"
                >
                  <Share2 size={20} />
                </button>
              </div>
          ...
          {selectedPlaylist ? (
            ...
            >
              <button
                ...
                className="flex items-center gap-3 text-white/40 hover:text-white font-black text-sm transition-all group"
              >
                <ArrowRight className="group-hover:-translate-x-2 transition-transform" />{" "}
                العودة للقوائم
              </button>
              <div className="flex flex-col md:flex-row gap-8 items-center md:items-end mb-16">
                <div className="w-20 h-20 bg-purple-600/20 rounded-3xl flex items-center justify-center text-purple-400 border border-purple-500/20">
                  <ListVideo size={32} />
                </div>
                <div>
                  ...
                  <p className="text-white/40 font-bold mt-2">
                    تتضمن هذه السلسلة {playlists[selectedPlaylist].length} أجزاء
                    معرفية.
                  </p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                  {Object.entries(playlists).map(([name, pPosts], idx) => (
                    <button
                      ...
                      className="text-right group block"
                    >
                      <div className="aspect-video bg-[#0d0d0d] rounded-[2.5rem] overflow-hidden mb-6 relative border border-white/5 group-hover:border-purple-500/30 transition-all shadow-2xl">
                        <OptimizedImage
                          src={getFullImageUrl(pPosts[0]?.cover_image_url)}
                          alt={name}
                          width={600}
                          height={337}
                  ...
                  {Object.keys(playlists).length === 0 && (
                    <EmptyState message="لم يتم إنشاء قوائم تشغيل لهذه القناة بعد." />
                  )}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                  ...
                  {posts.length === 0 && (
                    <EmptyState message="لا توجد منشورات حالياً في هذه القناة." />
                  )}
                </div>
              )}
            </motion.div>
    ...
    >
      <Link to={`/post/${post.slug}`} className="block">
        <div className="aspect-[16/10] bg-[#0d0d0d] rounded-[2.5rem] overflow-hidden mb-6 border border-white/5 group-hover:border-purple-500/30 transition-all shadow-2xl relative">
          ...
          />
          {isPlaylist && (
            <div className="absolute top-4 right-4 bg-purple-600 px-4 py-1.5 rounded-full text-[10px] font-black shadow-xl">
              الجزء {index + 1}
            </div>
          )}
        ...
        <h3 className="text-xl md:text-2xl font-black mb-3 leading-tight group-hover:text-purple-400 transition-colors line-clamp-2">
          {post.title}
        </h3>
        <div className="flex items-center gap-4 text-[10px] font-black text-white/60 uppercase tracking-widest">
          <span className="flex items-center gap-1.5">
            <Clock size={14} /> {formatDate(post.published_at)}
          </span>
        </div>
      </Link>
    </motion.article>
