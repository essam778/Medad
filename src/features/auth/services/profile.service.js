...
export const ProfileService = {
  ...
  async getProfile(id) {
    return await supabase
      .from("profiles")
      .select(
        "id, full_name, avatar_url, bio, role, points, is_banned, created_at, site_settings(channel_slug, site_name)",
      )
      .eq("id", id)
      .maybeSingle();
  },
  ...
  async getPublicProfile(id) {
    return await supabase
      ...
      .select(
        "id, full_name, avatar_url, bio, role, points, created_at, site_settings(channel_slug, site_name)",
      )
      .eq("id", id)
      .maybeSingle();
  },
  ...
  async deleteUser(userId) {
    ...
    return await supabase.rpc("delete_user_by_admin", {
  ...
  async updatePoints(id, points) {
    return await supabase.from("profiles").update({ points }).eq("id", id);
  },
  ...
  async createCreatorRequest(userId, message) {
    // Check for recent rejected request (within 30 days)
    ...
    if (recentRequest && recentRequest.length > 0) {
      throw new Error(
        "لا يمكنك تقديم طلب جديد الآن. يجب مرور 30 يوماً على الأقل من تاريخ رفض طلبك السابق.",
      );
    }
  ...
  async rejectCreatorRequest(requestId, userId) {
    ...
    if (!res.error && userId) {
      NotificationService.notifyUser({
        ...
        type: "system",
        title: "طلب الانضمام كصانع محتوى",
        message:
          "عذراً، لم يتم قبول طلبك للانضمام كصانع محتوى في الوقت الحالي. يمكنك التقديم مجدداً بعد 30 يوماً.",
