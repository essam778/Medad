...
describe('CommentSection', () => {
  ...
  describe('Authenticated user — form', () => {
    ...
    it('submit button is disabled when textarea is empty', () => {
      renderSection(authProps)
      expect(screen.getByRole('button', { name: /نشر التعليق/ })).toBeDisabled()
    })
  ...
  describe('Comment submission', () => {
    ...
    it('calls PostService.addComment with correct args', async () => {
      ...
      expect(mockAddComment).toHaveBeenCalledWith('post-1', 'user-1', 'تعليق جديد', undefined)
    })
    ...
    it('adds new comment to the list after success', async () => {
      ...
      await user.type(screen.getByRole('textbox'), 'تعليق جديد')
      await user.click(screen.getByRole('button', { name: /نشر التعليق/ }))
      await waitFor(() => expect(screen.getByText('تعليق جديد')).toBeInTheDocument())
    })
    ...
    it('clears draft from localStorage after successful submit', async () => {
      ...
      await user.type(screen.getByRole('textbox'), 'تعليق جديد')
      await user.click(screen.getByRole('button', { name: /نشر التعليق/ }))
      await waitFor(() => expect(localStorage.getItem('comment_draft_post-1')).toBeNull())
    })
  ...
  describe('Rate limiting', () => {
    ...
    it('shows NoticeModal when submitting too fast', async () => {
      mockAddComment.mockResolvedValue(makeComment({ id: 'c-1', content: 'أول تعليق' }))
      const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime })
      renderSection(authProps)
      ...
      mockAddComment.mockResolvedValue(makeComment({ id: 'c-2', content: 'تعليق سريع' }))
  ...
  describe('Reply flow', () => {
    const authProps = { user: mockUser, profile: mockProfile }
    const parentComment = makeComment({ id: 'parent-1', content: 'تعليق أصلي' })
    ...
    it('cancel reply button clears replyingTo', async () => {
      ...
      renderSection({ ...authProps, initialComments: [parentComment] })
      await user.click(screen.getByRole('button', { name: 'رد' }))
      await waitFor(() => expect(screen.getByText(/جاري الرد على/)).toBeInTheDocument())
    ...
    it('passes parent_id when submitting a reply', async () => {
      mockAddComment.mockResolvedValue(
        makeComment({ id: 'reply-1', content: 'رد على التعليق', parent_id: 'parent-1' })
      )
      const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime })
      renderSection({ ...authProps, initialComments: [parentComment] })
      await user.click(screen.getByRole('button', { name: 'رد' }))
      ...
      await user.click(screen.getByRole('button', { name: /نشر التعليق/ }))
      await waitFor(() =>
        expect(mockAddComment).toHaveBeenCalledWith('post-1', 'user-1', 'رد على التعليق', 'parent-1')
      )
    })
  })
  ...
  describe('Nested comments tree', () => {
    it('renders top-level comments only at root', () => {
      ...
      const reply   = makeComment({ id: 'r1', content: 'رد على التعليق', parent_id: 'p1' })
    ...
    it('does not show Reply button at depth ≥ 3', () => {
      ...
      renderSection({ user: mockUser, profile: mockProfile, initialComments: [p1, p2, p3, p4] })
      // All rendered
