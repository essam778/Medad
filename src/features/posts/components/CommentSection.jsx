...
import { MessageCircle, Send, LogIn, Clock } from 'lucide-react'
...
export function CommentSection({ postId, initialComments = [], user, profile, onCommentAdded, disabled = false }) {
        ...
        return (
          ...
          >
            <div className="flex gap-4 md:gap-6">
              <Link to={`/u/${comment.user_id}`} className={`rounded-2xl overflow-hidden bg-[#0d0d0d] flex-shrink-0 border border-white/5 group-hover:border-purple-500/30 transition-all shadow-xl ${depth > 0 ? 'w-10 h-10 md:w-12 md:h-12' : 'w-12 h-12 md:w-16 md:h-16'}`}>
                {comment.profiles?.avatar_url ? (
                  <OptimizedImage src={comment.profiles.avatar_url} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <Link to={`/u/${comment.user_id}`} className="font-black text-xs md:text-sm text-white hover:text-purple-400 transition-colors truncate">
                  ...
                  {user && depth < 3 && ( // Limit nesting to 3 levels for UI sanity
                    <button 
                      onClick={() => { 
  ...
  return (
    <section className="pt-24 border-t border-white/5 mb-32">
      <h2 className="text-3xl font-black mb-16 flex items-center gap-4 italic">
        <MessageCircle className="text-purple-500" size={32} /> النقاشات ({comments.length})
      </h2>
      
      {disabled ? (
        <div className="p-12 bg-white/5 rounded-[3rem] text-center border border-dashed border-white/10 mb-16">
           <p className="text-white/40 font-black italic text-lg">التعليقات مغلقة لهذا المقال من قِبل المبدع.</p>
        </div>
      ) : user ? (
        <form onSubmit={handleSubmit} className="mb-20 bg-[#0d0d0d]/50 backdrop-blur-3xl rounded-[3rem] p-8 md:p-12 border border-white/5 focus-within:border-purple-500/30 transition-all shadow-2xl relative overflow-hidden">
            ...
            <div className="flex-1 space-y-4">
              {replyingTo && (
                <div className="flex items-center justify-between bg-purple-600/10 text-purple-400 p-3 rounded-2xl border border-purple-500/20">
                  <span className="text-xs font-black">جاري الرد على {replyingTo.profiles?.full_name}</span>
                  <button type="button" onClick={() => setReplyingTo(null)} className="text-purple-400/50 hover:text-purple-400 font-bold text-xs">إلغاء</button>
                </div>
              )}
              <textarea 
                id="comment-input"
                ...
                placeholder={replyingTo ? "اكتب ردك هنا..." : "بماذا تفكر؟ أضف رأيك هنا..."}
        ...
        >
          ...
          <p className="text-white/40 font-black italic text-xl mb-10">يجب عليك تسجيل الدخول لتتمكن من الانضمام للنقاش.</p>
          <Link to="/login" className="bg-white text-black px-12 py-5 rounded-2xl font-black shadow-2xl inline-flex items-center gap-3 hover:scale-105 transition-all">
