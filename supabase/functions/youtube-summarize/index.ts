    ...
    if (url && (url.includes("youtube.com") || url.includes("youtu.be"))) {
      ...
      try {
        ...
        if (legacyXml.includes("track")) {
          ...
          const lang = legacyXml.includes('lang_code="ar"')
            ? "ar"
            : langMatch
              ? langMatch[1]
              : "en";
        ...
        const ytResponse = await fetch(
          `https://www.youtube.com/watch?v=${videoId}`,
          {
            headers: {
              ...
              Cookie:
                "CONSENT=YES+cb.20210328-17-p0.en+FX+412; VISITOR_INFO1_LIVE=f7p9p9p9p9; ",
      ...
      if (!contentToSummarize) {
        try {
          const ytTranscriptRes = await fetch(
            `https://youtubetranscript.com/api?vid=${videoId}`,
          );
          const ytTranscriptData = await ytTranscriptRes.json();
          if (ytTranscriptData?.transcript?.length > 0) {
            contentToSummarize = ytTranscriptData.transcript
      ...
      if (!contentToSummarize) {
        throw new Error(
          "يوتيوب يرفض إعطاء النص حالياً. يرجى تجربة فيديو آخر أو نسخ النص يدوياً.",
        );
      }
          ...
          contents: [
            {
              parts: [
                {
                  text: `حول النص التالي لمقال عربي احترافي بصيغة JSON: { "title": "...", "article": "..." }. النص: ${contentToSummarize.slice(0, 12000)}`,
                },
              ],
            },
          ],
